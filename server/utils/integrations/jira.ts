 ...
 */
export async function transitionJiraIssue(config: IJiraConfig, issueKey: string, status: string): Promise<void> {
    ...
    const transitionsResponse = await fetch(`${config.baseUrl}/rest/api/3/issues/${issueKey}/transitions`, {
        method: 'GET',
        headers: {
            Authorization: `Basic ${auth}`,
            'Content-Type': 'application/json',
    ...
    const transition = transitionsData.transitions.find((t: IJiraTransition) => t.to.name === targetStatus)
    ...
    const response = await fetch(`${config.baseUrl}/rest/api/3/issues/${issueKey}/transitions`, {
        method: 'POST',
        headers: {
            Authorization: `Basic ${auth}`,
            'Content-Type': 'application/json',
